...
public:
  TVersion GetVersion() const override {
    return Version;
  }

  TValuePtr Get(const TKey &) const override {
...
public:
  ...
  void DumpLog() const override {
    ...
    for (const auto& [key, value] : *Memtable) {
      ...
      keys.push_back(TKeyLogEntry{.Key = key, .Position = pos, .Size = newPos - pos});
      pos = newPos;
    }
...
class TLogReader final : public ILogReader {
    ...
    std::string ValuesFilePath;
public:
  std::vector<TKey> ReadKeys() const override {
    return {};
  }
  TValuePtr ReadValue(size_t i) const override {
    return {};
  }
};
...
TLogWriterPtr CreateLogWriter(std::string keysFilePath, std::string valuesFilePath, const TMemtablePtr & memtable) {
    return std::make_unique<TLogWriter>(keysFilePath, valuesFilePath, memtable);
}

TLogWriterPtr CreateLogReader(std::string keysFilePath, std::string valuesFilePath) {
    return std::make_unique<TLogReader>(keysFilePath, valuesFilePath);
}
